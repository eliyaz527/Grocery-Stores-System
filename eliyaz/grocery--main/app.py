...
'''@app.route('/pay_all', methods=['GET', 'POST'])
def pay_all():
    if not session.get('user'):
        return redirect(url_for('login'))

    items_in_cart = session.get(session.get('user'))

    # Prepare data for checkout session
    line_items = []
    total_amount = 0

    for item_id, item_details in items_in_cart.items():
        name = item_details[0]
        price = int(item_details[3])
        quantity = item_details[2]
    ...
    # Create a Checkout Session (outside the loop)
    checkout_session = stripe.checkout.Session.create(
        success_url=url_for('success', itemid=item_id, name=name, q=quantity, total=total_amount, _external=True),
...
def pay_all():
    ...
    success_url = url_for('success', _external=True) + '?' + '&'.join([f'itemid={item_id}&name={item_details[0]}&q={item_details[2]}&total={item_details[3]}' for item_id, item_details in items_in_cart.items()])
...
@app.route('/cancel')
def cancel():
    # You can perform any necessary actions here, such as displaying a message
    # or redirecting the user to a specific page.
    return "Payment cancelled. Your order was not processed."
...
def success():
    if session.get('user'):
        ...
        itemids = request.args.getlist('itemid')
        names = request.args.getlist('name')
        quantities = request.args.getlist('q')
        totals = request.args.getlist('total')
        
        # Iterate over each item and add it to the orders
        for itemid, name, q, total in zip(itemids, names, quantities, totals):
            # Assuming you have a way to retrieve the item name based on the item ID
            cursor.execute('select item_name from additems where item_id=uuid_to_bin(%s)',[itemid])
            item_name=cursor.fetchone()[0]
            cursor.execute('INSERT INTO orders (itemid, item_name, qyt, total_price, user) VALUES (uuid_to_bin(%s), %s, %s, %s, %s)', [itemid, item_name, q, total, user])
